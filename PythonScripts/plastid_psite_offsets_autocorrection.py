...
if __name__ == "__main__":
	...
	#		print lgt_previous
		...
		#	print "WEIRD"
			...
			if x != len(l_poffset) - 1:
				if lowest_poffset in lgt1:
					changed_poffset_lgt = lgt1
					unchanged_poffset_lgt = lgt2
				else:
					changed_poffset_lgt = lgt2
					unchanged_poffset_lgt = lgt1
			else:
				#print "FIN"
				changed_poffset_lgt = lgt1
				unchanged_poffset_lgt = lgt2
			
		#	print changed_poffset_lgt
			
			list_metagene_info_lowest_poffset_lgt = []
			
			for g in dict_metagene_profiles[str(changed_poffset_lgt[0])]:
		...
		#	print list(reversed(sorted(list_metagene_info_lowest_poffset_lgt)))
			...
			for u in dict_metagene_profiles[str(changed_poffset_lgt[0])]:
				if u[0] == -changed_poffset_lgt[1]:
					metagene_info = u[1]
		#	print metagene_info
		...
		#		print "multiple metagene"
				...
				for e in dict_metagene_profiles[str(changed_poffset_lgt[0])]:
				...
				for t in range(len(p_offsets_new_higher_metagene_info)):
					if t > 0 and t < len(p_offsets_new_higher_metagene_info) - 1:
						...
						if current_diff_offsets_new_higher_metagene_info_previous_poffset - unchanged_poffset_lgt[1] < next_diff_offsets_new_higher_metagene_info_previous_poffset - unchanged_poffset_lgt[1]:
		...
		#		print new_poffset
				...
				if math.fabs(new_poffset - unchanged_poffset_lgt[1]) > diff_poffset:
					final_poffset = lowest_poffset[1]
				elif diff_poffset_current_previous < math.fabs(new_poffset - unchanged_poffset_lgt[1]):		
					final_poffset = lowest_poffset[1]
				else:
				...
				for e in dict_metagene_profiles[str(changed_poffset_lgt[0])]:
					if e[1] == new_higher_metagene_info:
						...
						#if new_poffset == changed_poffset_lgt[1]:
	...
	#					print new_poffset
						if math.fabs(new_poffset - unchanged_poffset_lgt[1]) > diff_poffset:
							final_poffset = changed_poffset_lgt[1]
						elif diff_poffset_current_previous != None:
							if diff_poffset_current_previous < math.fabs(new_poffset - unchanged_poffset_lgt[1]):
								final_poffset = changed_poffset_lgt[1]
							else:
								final_poffset = new_poffset
						else:
							final_poffset = new_poffset
		#				print final_poffset	
							#changed_poffset_lgt = (str(changed_poffset_lgt[0]), final_poffset)
						dict_final_poffset[str(changed_poffset_lgt[0])] = final_poffset
					
						if changed_poffset_lgt == lgt2:
							if x < len(l_poffset) - 1:
								l_poffset[x + 1] = (str(changed_poffset_lgt[0]), final_poffset)
