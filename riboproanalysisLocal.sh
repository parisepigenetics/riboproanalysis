...
if [ -z $SAMPLE_ARRAY ]
then
	echo "Give the samples array in $1 (eg : export SAMPLE_ARRAY=(sample1 sample2 ...))."
	exit 1
fi
...
then
	echo "Give the absolute path to your reference genome FASTA. in $1 (eg : export PATH_TO_REFERENCE_GENOME_FILE=/path/to/my/ref/genome/mygenome.fasta)"
	exit 1
fi

if [ -z $PATH_TO_REFERENCE_TRANSCRIPTOME_FILE ]
then
	echo "Give the absolute path to your reference transcriptome FASTA. in $1 (eg : export PATH_TO_REFERENCE_TRANSCRIPTOME_FILE=/path/to/my/ref/transcriptome/mytranscriptome.fasta)"
	exit 1
fi
...
then
	# echo $?
	exit 1 ## TODO Check if we can get the exit status of the last job
fi
